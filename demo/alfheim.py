...
def h_24_npc(data, stat):
    ...
    if coords == (12, 3):
        ...
        if stat[9] == -1 or data[0] == stat[9]:
            stat[9] = data[0]
            return [0, "Quel sort souhaitez-vous oublier ?\n" + "\n".join(["{0}. {1} {2}".format(nb + 1, spells[nb], levels[lvl - 1]) for nb, (name, lvl) in enumerate(stat[7])]), len(stat[7])]
    ...
    if coords == (36, 12):
        ...
        if len(stat[7]) >= 3: return [0, "Je suis desolé, vous ne pouvez pas apprendre plus de trois sorts."]
        ...
        if stat[9] == -1 or data[0] == stat[9]:
            ...
            for i in range(1, 5):
                if data[0] == stat[9] + i:
                    ...
                    spell_id = -1
                    for sp_id, sp in enumerate(stat[7]):
                    ...
                    return [-i, "[DIOMWAR OUVRIT UN LIVRE RELIE DE CUIR NOIR, L'OUVRIT ET TRACA DU DOIGT DES SIGNES CABALISTIQUES SUR LE SOL. LES RUNES BRILLERENT PUISSAMMENT AVANT DE S'ETEINDRE.]", 0, (1, -spells_sale[i - 1][1] * 10)]
